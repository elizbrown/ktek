...
// Initialize the OPF template
var opfTemplate = fs.readFileSync(path.resolve(__dirname,
    '../lib/book.mustache.opf'), 'utf8');
...
module.exports.parseFile = function (baseDir, fileName, callback) {
  ...
  fs.readFile(path.join(baseDir, fileName), 'utf8', function (err, data) {
    ...
    async.forEach(manifest.manifest, function (file, fcb) {
      ...
      callback(null, mustache.render(opfTemplate, {
        metaTags: metaTags.join('\n'),
        manifestTags: manifestTags.join('\n'),
        spineTags: spineTags.join('\n'),
        guideTags: guideTags.join('\n')
      }));
    });
  });
};
